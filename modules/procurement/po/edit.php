...
$(document).ready(function() {
    ...
    // Run price comparison checks for pre-loaded rows in edit form
    $('.po-row').each(function() {
        var row = $(this);
        var itemId = row.find('input[name="item_id[]"], select[name="item_id[]"]').val();
        if (itemId) {
            checkPriceComparison(row, itemId);
        }
    });
